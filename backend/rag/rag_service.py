...
class RAGService:
    ...
    # -------------------------
    # 시설명 검색 (동의어 없이 SQLite 기반)
    # -------------------------
    def search_facility_best_match(self, facility_name: str):
        search_name = facility_name.strip()
    ...
    def search_apartments_hybrid(self, facility_name: str = None, radius: int = 500,
        ...
        if not facility_name:
            if parsed:
                facility_name = parsed.get("facility_name") or parsed.get("school") or parsed.get("name")
        ...
        sql_results = self.search_apartments_sqlite(facility["lat"], facility["lng"], radius)

        # 벡터 검색 병합
        if query and self.apartment_col and len(query) > 10:
            try:
                vector_results = self.retrieve_apartments_vector(query, facility["lat"], facility["lng"], radius)

                existing = {x["apartment"] for x in sql_results}
                for apt in vector_results:
                    if apt["apartment"] not in existing:
                        sql_results.append(apt)
                        existing.add(apt["apartment"])
            except:
                pass
    ...
    def _search_between_mode(self, parsed: dict, radius: int = 500):
        ...
        dist_between = self._haversine(fac1["lat"], fac1["lng"], fac2["lat"], fac2["lng"])
        search_radius = min(dist_between / 2 + 200, radius)

        results = self.search_apartments_sqlite(mid_lat, mid_lng, int(search_radius))

        for apt in results:
            d1 = self._haversine(fac1["lat"], fac1["lng"], apt["lat"], apt["lng"])
            d2 = self._haversine(fac2["lat"], fac2["lng"], apt["lat"], apt["lng"])

            apt["distance_facility1"] = round(d1, 1)
            apt["distance_facility2"] = round(d2, 1)
            apt["distance_school"] = round((d1 + d2) / 2, 1)
